...
class Player(tk.Frame):
	...
	def create_frames(self):
		...
		self.track = tk.LabelFrame(self, text='SONG TRACK',
					...
					bg="green", fg="white", bd=0, relief=tk.GROOVE)
		...
		self.controls = tk.LabelFrame(self,
							font=("ms sans serif", 15, "bold"),
							bg="black", fg="white", bd=0, relief=tk.GROOVE)
		self.controls.config(width=410, height=80)
		self.controls.grid(row=2, column=0, padx=0)
	...
	def track_widgets(self):
		...
		self.songtrack = tk.Label(self.track, font=("ms sans serif", 16, "bold"),
						bg="black", fg="white")
		self.songtrack['text'] = 'MusicX MP3 Player'
	...
	def control_widgets(self):
		...
		self.loadSongs.grid(row=0, column=0, padx=7)

		self.prev = tk.Button(self.controls, image=prev)
		self.prev['command'] = self.prev_song
		self.prev.grid(row=0, column=1, padx=7)
		self.prev.configure(background='black', border='0')

		self.pause = tk.Button(self.controls, image=pause)
		self.pause['command'] = self.pause_song
		self.pause.grid(row=0, column=2, padx=7)
		self.pause.configure(background='black', border='0')

		self.next = tk.Button(self.controls, image=next_)
		self.next['command'] = self.next_song
		self.next.grid(row=0, column=3, padx=7)
		self.next.configure(background='black', border='0')
		...
		self.slider.configure(background='green', border='0')
	...
	def play_song(self, event=None):
		...
		self.list.itemconfigure(self.current, bg='lightgreen')
